...
public class Reader {
	...
	public static void main(String[] args) throws Exception {
		...
		//push mode uses
		List<Discussions> post = new ArrayList<Discussions>();
		// write to server
		...
		try {
			if (mode.equals("push")) {
				...
				for (int i = 0; i < pushSize; i++) {
					...
					post.add(new Discussions(Integer.parseInt(components[0]), components[1], components[2], 
		...
		while ((sentence = inFromUser.readLine()) != null ) {					
			...
			if (request == null) {
				...
				if (mode.equals("push") && request instanceof ReadReq) { 
					Response response = ((ReadReq) request).localProcess(post);
...
//		clientSocket.close();
	...
	private static class Time extends TimerTask {
		...
		private Time (TCP tcp, PageNew pageNew, List<Discussions> post, List<Integer> readPosts, 
		...
		public void run() {
			...
			try {
				while ((response = tcp.objectsInFromServer.readObject()) != null ) {
					if (response instanceof DisplayRes) {
						//update the last visited page
						pageNew.currentPage = ((DisplayRes) response).getPage();
						if (mode.equals("push")) {
							System.out.println(((DisplayRes) response).toString(pageNew.currentPage.getBookName(), 
									pageNew.currentPage.getPageNumber(), post, readPosts));
						}
						if (mode.equals("pull")) {
							System.out.println(((DisplayRes) response).toString());
							if (pingThread != null) {
								pingThread.cancel();
							}
					...
					else if (response instanceof NotificationPushing) {
						((NotificationPushing) response).updateLocalDB(post, readPosts);
						if (this.pageNew.currentPage != null) {
							System.out.print(((NotificationPushing) response).toString(pageNew.currentPage.getBookName(),
									pageNew.currentPage.getPageNumber()));
						}
					}
					else if (response instanceof RespPing) {
						if (((RespPing) response).getStatus() == true && this.pingNotNotified) {
							System.out.println(response.toString());
							this.pingNotNotified = false;
						}
						else if (((RespPing) response).getStatus() == false ) {
							this.pingNotNotified = true;
						}
					} else {
	...
	private static class Ping extends TimerTask {
		...
		public void run() {
			try {
				tcp.writeToStream(new ReqPing(this.userName + " pings " + this.bookName + " " + this.pageNumber, 
	...
	private static Request createRequest (String clientRequest, String userName, Pages mostRecentPage, StringBuilder errorMessage, List<Integer> readPosts) {
		...
		try {
			if (clientRequest.startsWith("display")) {
				if (clientRequest.split(" ").length < 3) {
					...
					request = new DisplayReq(userName+" requests a page", userName, clientRequest.split(" ")[1], Integer.parseInt(clientRequest.split(" ")[2]), readPosts);
				}
			}
				...
				if (mostRecentPage == null) {
					...
					request = new RequestThePosts("New post received from " + userName, userName, 
				...
				if (mostRecentPage == null) {
					...
					request = new ReadReq(userName+" read posts", userName, 
							mostRecentPage.getBookName(), mostRecentPage.getPageNumber(), 
